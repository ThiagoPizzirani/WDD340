...
--Tabel structure for table 'inventory'
...
--DELETE the Tony Stark record from accounts table
DELETE FROM 
    public.account 
WHERE 
    account_id = 1;

--UPDATE 'GM HUMMER' description record 
UPDATE 
    public.inventory
SET
...
--(NOT WORKING) INNER JOIN to select make and model fields from the inventory table from the 'Sports' category from the classification table 
SELECT
    inventory.inv_make,
    inventory.inv_model,
...
--UPDATE all records in the inventory table for image and thumbnail with new file path '/vehicles'
UPDATE 
    public.inventory
SET
